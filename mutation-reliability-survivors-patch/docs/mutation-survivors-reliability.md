# Analisis Survivor Mutation Testing — `reliability/` (Fase 3.5)

Berdasarkan `mutation-report.zip` (skor 91.15%, run CI terakhir): 368 killed, 13 timeout,
37 survived, 0 no-coverage, 13 error. Dokumen ini HANYA membahas 9 survivor di
`src/shared/reliability/` (`bulkhead.ts` 3, `circuit-breaker.ts` 5, `retry.ts` 1) —
...
sendiri. Survivor di file LAIN (`export.ts`, `cache.ts`, `cache-keys.ts`, `jwt.ts`,
`user-agent.ts`, `response.ts` — 22 total) belum dianalisis, di luar cakupan patch ini.
