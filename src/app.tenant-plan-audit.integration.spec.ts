...
/**
 * Temuan T2 — integration test end-to-end `PATCH /api/v1/tenants/:id/plan`: `createApp()`
 * sungguhan lewat `supertest` (auth JWT asli, RBAC asli, controller/service/repository asli),
 * Prisma di-mock. Endpoint ini sebelumnya BELUM PERNAH dijalankan sebagai satu kesatuan (hanya
 * unit test per lapisan). Membuktikan: perubahan plan tercatat di audit log (siapa, dari plan apa
 * ke plan apa), baris yang ditulis LULUS verifikasi hash chain lewat kode produksi asli, dan tidak
 * ada audit untuk aksi yang tidak terjadi (403/404/422/401).
 */
...
function buildHarness(): Harness {
  ...
  jest.isolateModules(() => {
    ...
    // Blacklist token berbasis DB: `null` = token tidak dicabut (deep mock default `undefined` dianggap dicabut).
    ...
    prismaMock.tenant.update.mockImplementation(((args: { data: { plan: string } }) =>
      Promise.resolve({ ...tenantRow, plan: args.data.plan })) as never);
...
describe('Integration: PATCH /api/v1/tenants/:id/plan + audit log (temuan T2)', () => {
  it('ADMIN mengubah PRO -> ENTERPRISE: 200, plan tersimpan, dan audit UPDATE pada Tenant tercatat dengan "dari -> ke"', async () => {
    const { app, prismaMock, sign } = buildHarness();

    const res = await request(app)
      .patch(`/api/v1/tenants/${TENANT_ID}/plan`)
    ...
    // Baris ikut hash chain (baris pertama chain: previousHash null).
    expect(typeof written.hash).toBe('string');
    expect(written.previousHash).toBeNull();
  });

  it('baris audit yang ditulis LULUS verifikasi hash chain lewat kode produksi asli; details yang diubah setelahnya TERDETEKSI', async () => {
    ...
    prismaMock.auditLog.findMany.mockResolvedValue([
      { ...written, details: '{"field":"plan","from":"PRO","to":"ENTERPRISE"}' },
    ] as never);
    await expect(repository.verifyChainIntegrity()).resolves.toMatchObject({
      valid: false,
      brokenAt: { id: written.id },
    });
  });

  it('role USER (tanpa tenant.manage) -> 403; plan TIDAK diubah dan TIDAK ada audit', async () => {
    const { app, prismaMock, sign } = buildHarness();
  ...
  it('tenant tidak ada -> 404, TIDAK ada audit untuk aksi yang tidak terjadi', async () => {
    ...
    const res = await request(app)
      .patch('/api/v1/tenants/tidak-ada/plan')
      .set('Authorization', `Bearer ${sign('ADMIN')}`)
      .send({ plan: 'FREE' });
  ...
  it('plan tidak valid -> 422, tidak ada perubahan dan tidak ada audit', async () => {
    const { app, prismaMock, sign } = buildHarness();
  ...
  it('BATAS YANG DIKETAHUI: kalau penulisan audit GAGAL, perubahan plan tetap berhasil (fail-open, perilaku AuditService untuk semua aksi)', async () => {
    const { app, prismaMock, sign } = buildHarness();
